# 出版企画書

編集者に渡すファイルは zip だけです。Chrome では左クリックせず、右クリック → 名前を付けてリンク先を保存。

https://github.com/kohnoda-glitch/kocorolab/raw/cursor/note-medium-writing-4caf/writing/drafts/book-proposal/出版企画書.zip

中身は `出版企画書.docx`。Word のまま送る。Google ドキュメントにするなら Drive に上げて「アプリで開く」。

画面で確認するだけなら:

https://htmlpreview.github.io/?https://github.com/kohnoda-glitch/kocorolab/blob/cursor/note-medium-writing-4caf/writing/drafts/book-proposal/OPEN-IN-BROWSER.html

再生成:

```bash
python3 writing/scripts/build_book_proposal.py
python3 writing/scripts/test_book_proposal.py
```
