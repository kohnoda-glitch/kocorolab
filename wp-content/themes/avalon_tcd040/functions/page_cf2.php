...
function show_page_meta_box($post, $options) {
  //global $post;
  //$options =  get_desing_plus_option();

  //キャッチフレーズ
  $headline = array( 'name' => __('Catchphrase', 'tcd-w'), 'desc' => '', 'id' => 'page_headline', 'type' => 'input', 'std' => '' );
  $headline_meta = esc_html(get_post_meta($post->ID, 'page_headline', true));
  ...
  //フォントの色
  $font_color = array( 'name' => __('Text Color', 'tcd-w'), 'desc' => '', 'id' => 'page_headline_color', 'type' => 'input', 'std' => 'FFFFFF' );
  $font_color_meta = esc_html(get_post_meta($post->ID, 'page_headline_color', true));

  //ドロップシャドウ
  $shadow1 = array( 'name' => __('The position of the horizontal shadow.', 'tcd-w'), 'desc' =>  __('If you want enter 0, please leave the field blank.', 'tcd-w'), 'id' => 'page_headline_shadow1', 'type' => 'input', 'std' => '0' );
  $shadow1_meta = esc_html(get_post_meta($post->ID, 'page_headline_shadow1', true));
  $shadow2 = array( 'name' => __('The position of the vertical shadow.', 'tcd-w'), 'desc' => __('If you want enter 0, please leave the field blank.', 'tcd-w'), 'id' => 'page_headline_shadow2', 'type' => 'input', 'std' => '0' );
  $shadow2_meta = esc_html(get_post_meta($post->ID, 'page_headline_shadow2', true));
  $shadow3 = array( 'name' => __('The blur radius.', 'tcd-w'), 'desc' => __('If you want enter 0, please leave the field blank.', 'tcd-w'), 'id' => 'page_headline_shadow3', 'type' => 'input', 'std' => '0' );
  $shadow3_meta = esc_html(get_post_meta($post->ID, 'page_headline_shadow3', true));
  $shadow4 = array( 'name' => __('The Color of the shadow.', 'tcd-w'), 'desc' => '', 'id' => 'page_headline_shadow4', 'type' => 'input', 'std' => '333333' );
  ...
  echo '<dd class="content"><p class="desc">' , __('Recommend image size. Width:1180px, Height:550px', 'tcd-w') , '</p>';
    mlcf_media_form('page_image', __('Image', 'tcd-w'));
  echo '</dd>';

  //キャッチフレーズ
  echo '<dt class="label"><label for="' , $headline['id'] , '">' , $headline['name'] , '</label></dt>';
  echo '<dd class="content"><input type="text" name="', $headline['id'], '" id="', $headline['id'], '" value="', $headline_meta ? $headline_meta : $headline['std'], '" size="30" style="width:100%" /></dd>';
...
function page_textstyle_e($post_id, $output_size = false) {
  $keyH = "page_headline_shadow1";
  $keyV = "page_headline_shadow2";
  $keyB = "page_headline_shadow3";
  $keyC = "page_headline_shadow4";
  $keyF = "page_headline_color";
  $keyS = "page_headline_font_size";
  echo "color: #".get_post_meta($post_id, $keyF, TRUE).";";
  if($output_size == true) {
    echo " font-size: " . get_post_meta( $post_id, $keyS, true ) . "px;";
  }
  $valH = get_post_meta( $post_id, $keyH, true );
  $valV = get_post_meta( $post_id, $keyV, true );
  $valB = get_post_meta( $post_id, $keyB, true );
  if((empty($valH) || $valH == "0") &&
     (empty($valV) || $valV == "0") &&
     (empty($valB) || $valB == "0")) {
    // シャドウの設定がない場合は処理しない
  } else {
    echo " text-shadow: " . $valH . "px " . $valV . "px " . $valB . "px #" . get_post_meta( $post_id, $keyC, true ) . ";";
  }
}
