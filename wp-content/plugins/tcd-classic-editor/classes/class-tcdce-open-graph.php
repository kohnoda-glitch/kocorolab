...
class TCDCE_Open_Graph implements Iterator
{
  /**
   ...
   */
	static public function fetch($URI) {
		...
		// Load cache
		if ( $can_cache ) {
			$cache_key = 'tcdce_opengraph_' . md5( $URI );
		...
		if ( $can_cache ) {
			...
			if ( function_exists( 'apply_filters' ) ) {
				$cache_seconds = apply_filters( 'tcdce_open_graph_fetch_cache_seconds', $cache_seconds );
			}
