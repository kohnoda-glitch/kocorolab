...
foreach ( array( 'hero-horizon.jpg', 'spirit-sky.jpg', 'society-green.jpg', 'environment-ocean.jpg', 'kocoro-mark-light.png', 'kohei-noda.jpg' ) as $img ) {
	copy( "$img_src/$img", "$img_dst/$img" );
}

$file_src = $root . '/wp-content/mu-plugins/kocorolab-site-refresh/files';
$file_dst = $out . '/files';
if ( ! is_dir( $file_dst ) ) {
	mkdir( $file_dst, 0755, true );
}
foreach ( glob( $file_src . '/*' ) as $path ) {
	if ( is_file( $path ) ) {
		copy( $path, $file_dst . '/' . basename( $path ) );
	}
}
